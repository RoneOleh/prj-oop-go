...
type MeasurementRepository interface {
	...
	FindByDeviceAndPeriod(deviceId uint64, from time.Time, to time.Time) ([]domain.Measurement, error)
...
func (r measurementRepository) FindByDeviceAndPeriod(deviceId uint64, from time.Time, to time.Time) ([]domain.Measurement, error) {
	var data []measurement
	err := r.coll.
		Find(db.Cond{
			"device_id":       deviceId,
